...
void* queueFileForDecrypt(void* parameters)
{
   ...
   while(*(params->running) && *(params->error) == 0 && bytes_read < orig_file_size)
   {
       ...
       if(data_chunk == NULL)
       {
           ...
           if(header)
           {
               chunk_size = header_byte_size;
           } //2 block headers prepended to file
           else if(!header && (orig_file_size - (bytes_read + block_byte_size)) >= MAX_CHUNK_SIZE)
           {
               chunk_size = MAX_CHUNK_SIZE;
           } //A full chunk of data
           else if(!header && (orig_file_size - (bytes_read + block_byte_size)) < MAX_CHUNK_SIZE &&
...
//Queing a file for Encrypt is easy all we have to do is read it into chunks and queue them until
...
void* queueFileForEncrypt(void* parameters)
{
    ...
    while(*(params->running) && *(params->error) == 0 && bytes_read < orig_file_size)
    {
        ...
        if(data_chunk == NULL)
	    {
            ...
            const uint64_t chunk_size = ((orig_file_size - bytes_read) >= MAX_CHUNK_SIZE) ? MAX_CHUNK_SIZE :
