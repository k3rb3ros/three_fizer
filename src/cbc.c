...
void cbc256Decrypt(ThreefishKey_t* key, uint64_t* iv, uint64_t* cipher_text, uint64_t num_blocks)
{
    ...
    for(uint64_t block=0; block<num_blocks; block+=SAFE_SLICE) //run each block through the cipher (in decrypt mode)
    {
        ...
        else
        {
            ...
            cipher_text[block] ^= cipher_text[offset]; cipher_text[block+1] ^= cipher_text[offset+1];
            cipher_text[block+2] ^= cipher_text[offset+2]; cipher_text[block+3] ^= cipher_text[offset+3];
        }*/
    }
}
...
void cbc512Decrypt(ThreefishKey_t* key, uint64_t* iv, uint64_t* cipher_text, uint64_t num_blocks)
{
    ...
    for(uint64_t block=0; block<num_blocks*SECURE_SLICE; block+=SECURE_SLICE) //run each block through the cipher (in decrypt mode)
    {
        ...
        uint64_t offset = block-SAFE_SLICE; //calculate the offset programatically to avoid arithmetic error
...
{
    for(uint64_t block=0; block<num_blocks; block+=FUTURE_PROOF_SLICE) //run each block through the cipher (in decrypt mode)
    {
        threefishDecryptBlockWords(key, &cipher_text[block], &cipher_text[block]); //decrypt the current block
        if(block == 0) //Xor the first plain text block with the plain_text
        {
            cipher_text[0] ^= iv[0]; cipher_text[1] ^= iv[1]; cipher_text[2] ^= iv[2]; cipher_text[3] ^= iv[3];
            cipher_text[4] ^= iv[4]; cipher_text[5] ^= iv[5]; cipher_text[6] ^= iv[6]; cipher_text[7] ^= iv[7];
        ...
        else
        {
            ...
            cipher_text[block] ^= cipher_text[offset]; cipher_text[block+1] ^= cipher_text[offset+1];
            cipher_text[block+2] ^= cipher_text[offset+2]; cipher_text[block+3] ^= cipher_text[offset+3];
            cipher_text[block+4] ^= cipher_text[offset+4]; cipher_text[block+5] ^= cipher_text[offset+5];
            cipher_text[block+6] ^= cipher_text[offset+6]; cipher_text[block+7] ^= cipher_text[offset+7];
            cipher_text[block+8] ^= cipher_text[offset+8]; cipher_text[block+9] ^= cipher_text[offset+9];
            cipher_text[block+10] ^= cipher_text[offset+10]; cipher_text[block+11] ^= cipher_text[offset+11];
            cipher_text[block+12] ^= cipher_text[offset+12]; cipher_text[block+13] ^= cipher_text[offset+13];
            cipher_text[block+14] ^= cipher_text[offset+14]; cipher_text[block+15] ^= cipher_text[offset+15];
        }
    }
}
