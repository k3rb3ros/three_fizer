...
int runThreefizer(const arguments* args)
{
    ...
    pdebug("Arguments { ");
    pdebug("free: %d, encrypt: %d, hash: %d, file_path: [%s], file_path_len: %zu, "
           "State Size: %u, password: [%s], pw_length %lu, file_length %lu }\n",
    ...
    if(handleKeys(args, &tf_key, &mac_context)) //generate and initialize the keys we cannot continue if this fails
    {
        ...
        const uint8_t* temp_file_name = binToHex(hash((uint8_t*)args->target_file,
                                                 TEMP_FILE_NAME_SIZE, args->state_size), TEMP_FILE_NAME_SIZE);
        ...
        {
            //pass through queues with no encryption
            setUpReadParams(&read_params, args, &running, &crypto_mutex, crypto_queue, &progress, &error);
            setUpCryptoParams(&crypto_params, args, &running, &tf_key, &crypto_mutex, &mac_mutex, crypto_queue,
                              mac_queue, &progress, &error);
            setUpMacParams(&mac_params, &mac_status, &running, NULL, &mac_context, &mac_mutex, &write_mutex,
                           mac_queue, write_queue, &progress, NULL, &error, NULL);
            setUpWriteParams(&write_params, args, &running, NULL, &write_mutex, write_queue, temp_file_name,
                             &progress, &error, NULL);
            //generate and queue the Header before we start reading the file so it is the first thing in the queue
        ...
        else //decrypt
        {
            ...
            setUpMacParams(&mac_params, &mac_status, &running, &valid, &mac_context, &mac_mutex,
                           &crypto_mutex, mac_queue, crypto_queue, &progress, &tf_key, &error, &file_size);
            setUpCryptoParams(&crypto_params, args, &running, &tf_key, &crypto_mutex, &write_mutex,
                              crypto_queue, write_queue, &progress, &error);
            setUpWriteParams(&write_params, args, &running, &valid, &write_mutex, write_queue,
                             temp_file_name, &progress,  &error, &file_size);
            threads_active = true;
        ...
        } //return the error if 1 occurred
