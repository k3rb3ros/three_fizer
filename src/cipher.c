...
{
    uint8_t* block = header;
    pdebug("Header as passed into check: [");
    for(uint16_t i=0; i<state_size/8; ++i)
    {
        pdebug("%c ", block[i]);
    }
    ...
    pdebug("header after decryption [");
...
uint64_t* gen_header(ThreefishKey_t* key, uint64_t data_size, uint64_t* iv, SkeinSize_t state_size) 
{
    ...
    if(iv != NULL && key != NULL && data_size > 0)
    {
        ...
        EncryptInPlace(key, 1, iv, header, state_size); //decrypt the cipher text

        pdebug("header as encrypted [");
        for(uint64_t i=0; i<(state_size/8); ++i) 
        {
            printf("%c ", disp[i]);
        }
